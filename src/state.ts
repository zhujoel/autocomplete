/* eslint-disable prefer-const, eqeqeq, @typescript-eslint/no-use-before-define, @typescript-eslint/no-this-alias, @typescript-eslint/strict-boolean-expressions, unicorn/no-this-assignment */
...
export class CompletionState {
  ...
  update(tr: Transaction) {
    ...
    let sources =
      conf.override ||
      state
        .languageDataAt<CompletionSource | readonly (string | Completion)[]>('autocomplete', cur(state))
        .map(asSource);
...
function checkValid(
  validFor: undefined | RegExp | ((text: string, from: number, to: number, state: EditorState) => boolean),
  state: EditorState,
  from: number,
  to: number
) {
