/* eslint-disable prefer-const, eqeqeq, @typescript-eslint/strict-boolean-expressions */
...
export function moveCompletionSelection(forward: boolean, by: 'option' | 'page' = 'option'): Command {
  return (view: EditorView) => {
    ...
    if (by == 'page' && (tooltip = getTooltip(view, cState.open.tooltip))) {
      step = Math.max(
        2,
        Math.floor(tooltip.dom.offsetHeight / (tooltip.dom.querySelector('li') as HTMLElement).offsetHeight) - 1
      );
    }
