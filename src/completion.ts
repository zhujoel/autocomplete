/* eslint-disable @typescript-eslint/strict-boolean-expressions, eqeqeq */
...
export class CompletionContext {
  ...
  tokenBefore(types: readonly string[]) {
    ...
    while (token && !types.includes(token.name)) {
      token = token.parent;
    }
    return token
      ? {
...
function completeFromList(list: readonly (string | Completion)[]): CompletionSource {
  ...
  const [validFor, match] = options.every((o) => /^\w+$/.test(o.label)) ? [/\w*$/, /\w+$/] : prefixMatch(options);
